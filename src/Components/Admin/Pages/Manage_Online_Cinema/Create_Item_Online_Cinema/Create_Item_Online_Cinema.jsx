...
const Create_Item_Online_Cinema = () => {
  ...
  const [genre2, Set_genre2] = useState([]);
  ...
  const create_item = async () => {
    const genres = [
      ...genre
        .split(",")
        .map((genre, index) => ({ id: index + 1, genre: genre.trim() })),
      { id: genre2.length > 0 ? genre.length + 1 : 0, genre: genre2.trim() },
    ];
    try {
...
export default Create_Item_Online_Cinema;
